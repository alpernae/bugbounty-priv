...
Use this reference to understand the full source-code security scan workflow after the executable phase skill has routed into source security scanning. It preserves the detailed orchestration rules, artifact expectations, validation checkpoints, false-positive gates, final report shape, impact handling, and remediation handoff guidance.

## Workflow

1. Resolve scan target and artifact paths.
2. Run threat modeling before candidate discovery.
3. Run finding discovery with diff-scoped or repository-wide rules.
4. Validate candidates and suppress false positives with counterevidence.
5. Run attack-path analysis for surviving findings.
6. Assemble the final report with impact, severity, evidence, remediation, and proof gaps.
...
|---|---|---|
| 1 | `../threat-model/SKILL.md` | Repository threat model or provided authoritative model. |
| 2 | `../finding-discovery/SKILL.md` | Candidate inventory or explicit no-findings result. |
| 3 | `../validation/SKILL.md` | Validation assessment and closure table for candidates. |
| 4 | `../attack-path-analysis/SKILL.md` | Attack-path facts, severity, and final policy decision. |
| 5 | `references/final-report.md` | Final report assembled from surviving findings. |

Use `../../references/scan-artifacts.md` to resolve default artifact paths. If the user provides explicit paths, use those.
