...
| OWASP / WSTG / API mapping | WSTG-CLNT-06 |

## What to look for

Legacy JSONP endpoint reflects a callback name as executable JavaScript without strict validation.

## Vulnerable request

```http
GET /api/profile.jsonp?callback=alert%281%29// HTTP/1.1
Host: target.example
```
...
Capture endpoint, content type, callback reflection, and whether authenticated data is exposed cross-origin.

## Remediation direction

Remove JSONP, require CORS with exact origins, and allowlist callback names with strict identifier regex only if legacy support is unavoidable.
