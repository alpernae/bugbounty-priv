...
| Field | Value |
| --- | --- |
| HackerOne weakness | Improper Neutralization of Script-Related HTML Tags in a Web Page |
| CWE / external ID | CWE-80 |
| OWASP / WSTG / API mapping | WSTG-CLNT-03 |

## What to look for

User-controlled HTML is rendered without script execution but can alter page content or facilitate phishing/UI abuse.

## Vulnerable request

```http
GET /welcome?name=<h1>Security Proof</h1><form>fake login</form> HTTP/1.1
Host: target.example
```
...
Welcome <h1>Security Proof</h1><form>fake login</form>
```
...
Show visible page manipulation and whether the injected content appears in trusted origin.

## Remediation direction

HTML-encode output and only allow rich text via a strict sanitizer and allowlisted tags.
