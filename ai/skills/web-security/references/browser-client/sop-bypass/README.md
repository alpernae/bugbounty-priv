# Same-Origin Policy Bypass Pattern
...
| Field | Value |
| --- | --- |
| HackerOne weakness | Privacy Violation |
| CWE / external ID | CWE-359 |
| OWASP / WSTG / API mapping | WSTG-CLNT-07 |

## What to look for

An endpoint or browser integration accidentally exposes authenticated cross-origin data through JSONP, CORS, postMessage, or XS-Leak behavior.

## Vulnerable request

```http
GET /api/private?format=jsonp&callback=leak HTTP/1.1
Host: target.example
...
leak({"email":"victim@example.com","plan":"pro"});
```
...
{"error":"jsonp_not_supported_for_private_data"}
```

## Evidence to collect

Show cross-origin primitive and exact data leaked without user interaction beyond authenticated state.

## Remediation direction

Disable JSONP for private endpoints, harden CORS/postMessage, and set CORP/COOP/COEP where appropriate.
