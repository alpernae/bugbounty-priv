...
Server redirects users to an attacker-controlled URL from a user-controlled parameter.

## Vulnerable request

```http
GET /login?next=https://attacker.example/callback HTTP/1.1
Host: target.example
```
...
Capture redirect chain, trusted source URL, destination, and whether auth tokens/codes leak in the redirect.

## Remediation direction

Allow only relative paths or strict allowlisted origins; reject scheme-relative URLs and encoded bypasses.
