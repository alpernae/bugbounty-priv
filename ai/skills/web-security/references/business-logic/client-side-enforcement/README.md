...
| Field | Value |
| --- | --- |
| HackerOne weakness | Client-Side Enforcement of Server-Side Security |
| CWE / external ID | CWE-602 |
| OWASP / WSTG / API mapping | WSTG-CLNT-12 |

## What to look for

Server trusts frontend-hidden fields or disabled controls for security decisions.
...
{"to":"friend","amount":1000,"is_limit_checked":true}
```
...
{"status":"sent","amount":1000,"limit_checked":"client"}
```
...
{"error":"server_side_limit_exceeded"}
```

## Evidence to collect

Show that modifying hidden/disabled/client-only value affects server behavior.

## Remediation direction

Treat all client input as untrusted and enforce limits, roles, and workflow state server-side.
