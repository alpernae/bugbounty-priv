...
| Field | Value |
| --- | --- |
| HackerOne weakness | Race Condition |
| CWE / external ID | CWE-362 |
| OWASP / WSTG / API mapping | WSTG-BUSL-10 |

## What to look for

Concurrent requests bypass one-time use, balance, inventory, or rate constraints.
...
Idempotency-Key: race-test-1

{"coupon":"ONCE_ONLY"}

# Same request sent twice concurrently in a controlled two-request test.
```
...
{"redemption_id":"r1","discount":"10.00"}

--- concurrent response ---
HTTP/1.1 200 OK
{"redemption_id":"r2","discount":"10.00"}
```
...
Use only low-volume controlled concurrency and test assets. Show duplicate success caused by race window.

## Remediation direction

Use database locks/transactions, unique constraints, idempotency keys, and atomic state transitions.
