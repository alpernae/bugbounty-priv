...
Server makes an outbound request that is not reflected in the response.
...
{"callback_url":"https://collab.example/ssrf-proof"}
```
...
# Collaborator receives: GET /ssrf-proof from target egress IP
```
...
{"error":"callback_domain_not_verified"}
```

## Evidence to collect

Capture collaborator request metadata only; no sensitive response body is expected.

## Remediation direction

Require pre-registered verified domains, sign callbacks, and restrict egress.
