...
| Field | Value |
| --- | --- |
| HackerOne weakness | Server-Side Request Forgery (SSRF) |
| CWE / external ID | CWE-918 |
| OWASP / WSTG / API mapping | WSTG-INPV-19 |

## What to look for

Server fetches a URL supplied by the user and can reach unintended internal or external resources.
...
{"title":"Internal Admin Status","body":"admin_service=online"}
```
...
{"error":"private_ip_ranges_not_allowed"}
```

## Evidence to collect

Use controlled internal-like targets only if allowed; otherwise use a collaborator DNS/HTTP proof.

## Remediation direction

Allowlist destinations, block private/link-local ranges after DNS resolution, prevent redirects to blocked ranges, and isolate egress.
