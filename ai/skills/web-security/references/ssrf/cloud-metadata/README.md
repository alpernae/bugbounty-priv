# SSRF to Cloud Metadata - Safe Check
...
| OWASP / WSTG / API mapping | API7 SSRF |

## What to look for

SSRF can reach cloud metadata services. Only perform safe block-checks under explicit authorization.
...
metadata service reachable - response body redacted
```
...
{"error":"link_local_ip_not_allowed"}
```

## Evidence to collect

Do not retrieve credentials or tokens. Prove reachability only and redact any accidental metadata.

## Remediation direction

Block link-local/metadata IPs, enforce IMDSv2 or cloud equivalent, and restrict outbound network access.
