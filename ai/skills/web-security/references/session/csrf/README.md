...
| OWASP / WSTG / API mapping | WSTG-SESS-05 |

## What to look for

A state-changing request relies only on cookies and lacks an unpredictable anti-CSRF token or SameSite protection.
...
email=attacker-controlled%40example.com
```
...
{"error":"invalid_csrf_token_or_origin"}
```

## Evidence to collect

Show forged cross-origin request, missing/ignored token, and actual state change on a test account.

## Remediation direction

Use per-request/session CSRF tokens, validate Origin/Referer for browser requests, and set SameSite cookies.
