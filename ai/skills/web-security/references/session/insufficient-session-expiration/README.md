...
| Field | Value |
| --- | --- |
| HackerOne weakness | Insufficient Session Expiration |
| CWE / external ID | CWE-613 |
| OWASP / WSTG / API mapping | WSTG-SESS-07 |

## What to look for

Old session tokens continue working after logout, password change, or long inactivity.
...
GET /api/me HTTP/1.1
Host: target.example
Cookie: session=TOKEN_USED_BEFORE_LOGOUT
```
...
{"id":123,"status":"still_valid_after_logout"}
```
...
Show logout/password change event and later reuse of the same token.

## Remediation direction

Invalidate server-side session on logout and sensitive changes; enforce idle and absolute timeouts.
