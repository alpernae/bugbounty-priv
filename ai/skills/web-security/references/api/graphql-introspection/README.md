...
| OWASP / WSTG / API mapping | API Security Misconfiguration |

## What to look for

GraphQL introspection/schema is exposed in production and reveals types, mutations, and hidden fields.
...
{"errors":[{"message":"introspection_disabled"}]}
```

## Evidence to collect

Schema exposure alone may be informational; show impact by linking to sensitive mutations/fields only if authorized.

## Remediation direction

Disable introspection for untrusted users in production or enforce authz on schema discovery; keep resolver auth independent.
