# Information Disclosure via Debug Error
...
| Field | Value |
| --- | --- |
| HackerOne weakness | Information Exposure Through an Error Message |
| CWE / external ID | CWE-209 |
| OWASP / WSTG / API mapping | WSTG-ERRH-01 |

## What to look for

Errors expose stack traces, paths, secrets, queries, or internal service details.

## Vulnerable request

```http
GET /api/items?id=not-an-int HTTP/1.1
Host: target.example
```
...
ValueError at /api/items
File "/srv/app/db.py", line 42
DATABASE_URL=postgres://user:REDACTED@db.internal/app
```
...
{"error":"invalid_item_id","request_id":"req_123"}
```

## Evidence to collect

Redact secrets. Capture stack trace type, internal paths, and request ID.

## Remediation direction

Disable debug mode in production, return generic errors, and log details server-side only.
