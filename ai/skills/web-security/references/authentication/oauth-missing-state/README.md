...
OAuth login flow lacks state binding, allowing login CSRF or account linking confusion.
...
Host: target.example
Cookie: session=VICTIM_BROWSER_SESSION
```
...
HTTP/1.1 302 Found
Location: /dashboard
Set-Cookie: session=VICTIM_NOW_LINKED_TO_ATTACKER_PROVIDER; Secure; HttpOnly
```
...
{"error":"missing_or_invalid_oauth_state"}
```

## Evidence to collect

Use two owned accounts. Show absence of state in authorization request and effect on account linking/login.

## Remediation direction

Generate high-entropy state per flow, bind it to the user session, and validate it before token exchange.
