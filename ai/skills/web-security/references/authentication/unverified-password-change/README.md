...
| Field | Value |
| --- | --- |
| HackerOne weakness | Unverified Password Change |
| CWE / external ID | CWE-620 |
| OWASP / WSTG / API mapping | WSTG-ATHN-09 |

## What to look for

Password can be changed without current password, MFA, fresh session, or reset-token verification.
...
{"error":"current_password_or_recent_mfa_required"}
```

## Evidence to collect

Show current-password/MFA omission and successful change on owned account.

## Remediation direction

Require current password or recent MFA/re-authentication for sensitive account changes.
