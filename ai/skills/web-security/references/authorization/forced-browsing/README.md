...
| Field | Value |
| --- | --- |
| HackerOne weakness | Forced Browsing |
| CWE / external ID | CWE-425 |
| OWASP / WSTG / API mapping | WSTG-ATHZ-02 |

## What to look for

Restricted URLs are accessible by direct request despite being hidden from navigation.

## Vulnerable request

```http
GET /reports/export/all-users.csv HTTP/1.1
Host: target.example
Cookie: session=BASIC_USER_SESSION
```
...
{"error":"report_permission_required"}
```

## Evidence to collect

Show endpoint is not linked in UI but responds directly to unprivileged caller.

## Remediation direction

Authorize every route server-side and avoid relying on obscurity or frontend route guards.
