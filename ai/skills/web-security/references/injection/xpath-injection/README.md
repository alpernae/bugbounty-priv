...
| CWE / external ID | CWE-643 |
| OWASP / WSTG / API mapping | WSTG-INPV-09 |

## What to look for

User input is inserted into XPath expressions and changes XML query results.

## Vulnerable request

```http
POST /xml-login HTTP/1.1
Host: target.example
...
{"status":"authenticated","source":"xml_user_store"}
```
...
Show harmless logic predicate changes auth/search result on a test account.

## Remediation direction

Use safe XPath variable binding or strict escaping and schema validation.
