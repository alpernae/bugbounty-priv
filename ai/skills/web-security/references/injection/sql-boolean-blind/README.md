...
Boolean changes in injected input alter application response without visible SQL errors.

## Vulnerable request

```http
GET /products?id=10%20AND%201=1 HTTP/1.1
Host: target.example
```
...
--- compare ---
GET /products?id=10%20AND%201=2
...
Show true/false condition produces consistent content difference using harmless predicates.

## Remediation direction

Parameterize queries and enforce numeric typing server-side before database access.
