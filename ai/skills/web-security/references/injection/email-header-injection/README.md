...
| Field | Value |
| --- | --- |
| HackerOne weakness | CRLF Injection |
| CWE / external ID | CWE-93 |
| OWASP / WSTG / API mapping | WSTG-INPV-11 |

## What to look for

Email fields accept CRLF and inject additional mail headers.
...
email=researcher@example.com%0d%0aBcc:test@example.net&message=hello
```
...
{"status":"sent","debug_headers":["From: researcher@example.com","Bcc: test@example.net"]}
```
...
Use owned test recipient only. Capture generated headers or received proof mail without spam volume.

## Remediation direction

Validate email fields strictly and use mail libraries that separate addresses from headers.
