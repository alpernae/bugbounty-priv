...
User input is concatenated into SQL and database errors disclose query behavior.
...
SQL syntax error near "books" in query SELECT * FROM products WHERE category = [user input]
```
...
{"items":[],"message":"no products found"}
```

## Evidence to collect

Capture baseline and modified request, response difference, and sanitized error behavior. Do not dump production data.

## Remediation direction

Use parameterized queries, ORM safe APIs, least-privileged DB accounts, and generic errors.
