...
| CWE / external ID | CWE-94 |
| OWASP / WSTG / API mapping | OWASP A03 Injection / WSTG-INPV-18 |

## What to look for

User-controlled values are evaluated by expression engines such as SpEL, OGNL, MVEL, JEXL, or server-side rule/template engines.

## Vulnerable request

```http
POST /api/rules/preview HTTP/1.1
Host: target.example
...
Start with harmless arithmetic/string markers. Do not attempt class loading, process execution, filesystem access, network calls, or sandbox escape in production.

## Remediation direction

Disable expression evaluation for untrusted input, use a safe DSL/parser, restrict available objects/methods, escape template syntax, and test common engine-specific metacharacters.
