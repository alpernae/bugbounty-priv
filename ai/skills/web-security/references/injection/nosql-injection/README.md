...
| CWE / external ID | CWE-20 |
| OWASP / WSTG / API mapping | WSTG-INPV-05 |

## What to look for

JSON/operator input changes NoSQL query logic, commonly in login or filter endpoints.
...
{"status":"authenticated","user_id":"first_matching_user"}
```
...
{"error":"email_and_password_must_be_strings"}
```

## Evidence to collect

Use owned test app/account where possible. Show type confusion/operator injection and resulting auth/filter bypass.

## Remediation direction

Validate JSON schema/types, disallow operator objects from user input, and use safe query builders.
