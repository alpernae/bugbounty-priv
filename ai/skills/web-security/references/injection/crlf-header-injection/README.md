...
| OWASP / WSTG / API mapping | WSTG-INPV-15 |

## What to look for

CRLF characters in user input are inserted into response headers.

## Vulnerable request

```http
GET /download?filename=report.pdf%0d%0aX-Proof:%20crlf HTTP/1.1
Host: target.example
```
...
Show injected proof header and affected parameter. Avoid cache poisoning unless in-scope and safe.

## Remediation direction

Reject CR/LF in header-bound input, use framework header APIs safely, and encode filenames.
