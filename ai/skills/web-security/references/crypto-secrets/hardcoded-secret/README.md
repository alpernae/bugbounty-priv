# Hardcoded Secret in Client or Config
...
| Field | Value |
| --- | --- |
| HackerOne weakness | Use of Hard-coded Password |
| CWE / external ID | CWE-259 |
| OWASP / WSTG / API mapping | WSTG-INFO-05 |

## What to look for

Sensitive credentials, keys, or tokens are hardcoded in reachable client files/configs.

## Vulnerable request

```http
GET /config/app-config.json HTTP/1.1
Host: target.example
```
...
{"apiBase":"https://api.target.example","servicePassword":"REDACTED_PROOF"}
```
...
Do not use the secret. Redact value and show enough prefix/context to validate.

## Remediation direction

Remove hardcoded secrets, rotate exposed values, and move secrets to protected server-side secret management.
