...
| Field | Value |
| --- | --- |
| HackerOne weakness | Path Traversal |
| CWE / external ID | CWE-22 |
| OWASP / WSTG / API mapping | WSTG-ATHZ-01 |

## What to look for

File path parameter can escape intended directory and read unintended files.

## Vulnerable request

```http
GET /download?file=../../config/app.example HTTP/1.1
Host: target.example
...
Read only non-sensitive proof files where possible; redact secrets if accidentally exposed.

## Remediation direction

Normalize paths, enforce base directory containment, use file IDs instead of paths, and deny traversal sequences.
