...
| Field | Value |
| --- | --- |
| HackerOne weakness | Remote File Inclusion |
| CWE / external ID | CWE-98 |
| OWASP / WSTG / API mapping | WSTG-ATHZ-01 |

## What to look for

Application includes remote content from a URL supplied by user input.

## Vulnerable request

```http
GET /theme?include=https://attacker.example/safe-proof.txt HTTP/1.1
Host: target.example
```
...
{"error":"remote_includes_disabled"}
```

## Evidence to collect

Use harmless text content; do not host executable payloads.

## Remediation direction

Disable remote includes, use allowlisted local templates/assets, and isolate rendering workers.
